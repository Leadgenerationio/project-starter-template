'use client'

import { useState, useMemo } from 'react'
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { Select } from '@/components/ui/select'
import { Table, TableHeader, TableBody, TableHead, TableRow, TableCell } from '@/components/ui/table'
import { Loader2, ArrowRight, X } from 'lucide-react'
import { LEADVAULT_FIELDS } from '@/lib/utils/excel-parser'
import type { ColumnMapping, PreviewResponse } from '@/lib/types'

interface ColumnMappingStepProps {
  preview: PreviewResponse
  onConfirm: (mapping: ColumnMapping) => void
  onCancel: () => void
  importing: boolean
}

const UNMAPPED = '__unmapped__'

export function ColumnMappingStep({ preview, onConfirm, onCancel, importing }: ColumnMappingStepProps) {
  const [mapping, setMapping] = useState<ColumnMapping>(() => {
    // Initialize from suggested mapping
    const initial: ColumnMapping = {}
    for (const field of LEADVAULT_FIELDS) {
      initial[field.key] = preview.suggested_mapping[field.key] || ''
    }
    return initial
  })

  const usedHeaders = useMemo(() => {
    const used = new Set<string>()
    for (const header of Object.values(mapping)) {
      if (header) used.add(header)
    }
    return used
  }, [mapping])

  const requiredMissing = useMemo(() => {
    return LEADVAULT_FIELDS
      .filter((f) => f.required && !mapping[f.key])
      .map((f) => f.label)
  }, [mapping])

  const duplicates = useMemo(() => {
    const counts: Record<string, number> = {}
    for (const header of Object.values(mapping)) {
      if (header) counts[header] = (counts[header] || 0) + 1
    }
    return new Set(Object.keys(counts).filter((k) => counts[k] > 1))
  }, [mapping])

  const canConfirm = requiredMissing.length === 0 && duplicates.size === 0

  function handleFieldChange(fieldKey: string, csvHeader: string) {
    setMapping((prev) => ({
      ...prev,
      [fieldKey]: csvHeader === UNMAPPED ? '' : csvHeader,
    }))
  }

  // Build preview rows mapped through current mapping
  const mappedPreviewRows = useMemo(() => {
    return preview.sample_rows.map((row) => {
      const mapped: Record<string, string> = {}
      for (const field of LEADVAULT_FIELDS) {
        const csvHeader = mapping[field.key]
        mapped[field.key] = csvHeader ? (row[csvHeader] ?? '') : ''
      }
      return mapped
    })
  }, [preview.sample_rows, mapping])

  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>Map Columns</span>
            <Badge variant="secondary">{preview.total_rows} rows</Badge>
          </CardTitle>
          <CardDescription>
            Map your file&apos;s columns to LeadVault fields. Required fields are marked with a badge.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {LEADVAULT_FIELDS.map((field) => {
            const selectedHeader = mapping[field.key]
            const isDuplicate = selectedHeader && duplicates.has(selectedHeader)

            return (
              <div key={field.key} className="flex items-center gap-3">
                <div className="w-36 flex items-center gap-2 shrink-0">
                  <span className="text-sm font-medium">{field.label}</span>
                  {field.required && (
                    <Badge variant="destructive" className="text-[10px] px-1.5 py-0">
                      Required
                    </Badge>
                  )}
                </div>
                <ArrowRight className="h-4 w-4 text-muted-foreground shrink-0" />
                <div className="flex-1">
                  <Select
                    value={selectedHeader || UNMAPPED}
                    onChange={(e) => handleFieldChange(field.key, e.target.value)}
                    className={isDuplicate ? 'border-destructive' : ''}
                  >
                    <option value={UNMAPPED}>-- Select column --</option>
                    {preview.headers.map((header) => (
                      <option
                        key={header}
                        value={header}
                        disabled={usedHeaders.has(header) && mapping[field.key] !== header}
                      >
                        {header}
                        {usedHeaders.has(header) && mapping[field.key] !== header ? ' (used)' : ''}
                      </option>
                    ))}
                  </Select>
                </div>
              </div>
            )
          })}

          {requiredMissing.length > 0 && (
            <p className="text-sm text-destructive">
              Missing required fields: {requiredMissing.join(', ')}
            </p>
          )}
          {duplicates.size > 0 && (
            <p className="text-sm text-destructive">
              Duplicate column mappings: {Array.from(duplicates).join(', ')}
            </p>
          )}
        </CardContent>
      </Card>

      <Card>
        <CardHeader>
          <CardTitle>Preview (first {preview.sample_rows.length} rows)</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  {LEADVAULT_FIELDS.map((field) => (
                    <TableHead key={field.key} className={!mapping[field.key] ? 'text-muted-foreground/50' : ''}>
                      {field.label}
                      {!mapping[field.key] && ' (unmapped)'}
                    </TableHead>
                  ))}
                </TableRow>
              </TableHeader>
              <TableBody>
                {mappedPreviewRows.map((row, i) => (
                  <TableRow key={i}>
                    {LEADVAULT_FIELDS.map((field) => (
                      <TableCell
                        key={field.key}
                        className={!mapping[field.key] ? 'text-muted-foreground/40 italic' : ''}
                      >
                        {row[field.key] || (mapping[field.key] ? '-' : '')}
                      </TableCell>
                    ))}
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          </div>
        </CardContent>
      </Card>

      <div className="flex gap-3">
        <Button onClick={() => onConfirm(mapping)} disabled={!canConfirm || importing}>
          {importing ? (
            <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Importing...</>
          ) : (
            `Import ${preview.total_rows} Rows`
          )}
        </Button>
        <Button variant="outline" onClick={onCancel}>
          <X className="h-4 w-4 mr-2" /> Cancel
        </Button>
      </div>
    </div>
  )
}
